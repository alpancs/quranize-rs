...
impl JsQuranize {
    ...
    fn get_locations(&self, quran: &str) -> Vec<JsLocation> {
        self.quranize
            ...
            .into_iter()
            .map(|(i, j)| {
                let (s, a, aya) = self.quranize.saq_pairs[i as usize];
                let (l, mut r) = (j as usize, j as usize + quran.len());
                if let Some(c) = aya[r..].chars().next() {
                    if matches!(c, '\u{064B}'..'\u{0651}' | '\u{0670}') {
                        r += c.len_utf8()
                    }
                };
                JsLocation {
