...
pub(crate) const AYA_COUNT: usize = 6236;
pub(crate) const AYA_STARTS: [usize; 114] = [
...
fn iter_quran(raw: &str) -> impl Iterator<Item = (u8, u16, &str)> {
    ...
    (0..AYA_COUNT)
        ...
        .map(move |(i, aya_text)| {
            if (sura_number as usize) < AYA_STARTS.len() && i == AYA_STARTS[sura_number as usize] {
                aya_number = 1;
                sura_number += 1;
            } else {
                aya_number += 1;
